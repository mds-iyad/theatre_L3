...
CREATE VIEW LesRepresentations(noSpec, dateRep, promoRep, nbPlacesRestantes) AS
...
CREATE TABLE LesCategoriesTickets (
  libelleCat text,
  tauxReductionCat real,
  constraint pk_catick_lib primary key (libelleCat),
  constraint ck_catick_lib check (libelleCat in ('normal', 'adherent', 'senior', 'etudiant', 'militaire'))
);
